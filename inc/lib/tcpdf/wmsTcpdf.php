...
require_once(dirname(__FILE__).'/include/wms_tcpdf_font_data.php');
require_once(dirname(__FILE__).'/include/wms_tcpdf_fonts.php');
require_once(dirname(__FILE__).'/include/wms_tcpdf_colors.php');
require_once(dirname(__FILE__).'/include/wms_tcpdf_images.php');
require_once(dirname(__FILE__).'/include/wms_tcpdf_static.php');
...
class wms_TCPDF {
	...
	protected $cell_height_ratio = WMS_K_CELL_HEIGHT_RATIO;
	...
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false) {
		...
		$this->file_id = md5(WMS_TCPDF_STATIC::getRandomSeed('TCPDF' . $orientation . $unit . $serformat . $encoding));
		...
		$this->InFooter = false;
		$this->lasth = 0;
		$this->FontFamily = defined('WMS_PDF_FONT_NAME_MAIN')?WMS_PDF_FONT_NAME_MAIN:'helvetica';
		$this->FontStyle = '';
		$this->FontSizePt = 12;
		...
		WMS_TCPDF_FONTS::utf8Bidi(array(), '', false, $this->isunicode, $this->CurrentFont);
	...
	protected function setPageFormat($format, $orientation='P') {
		...
		if (is_string($format)) {
			$pf = WMS_TCPDF_STATIC::getPageSizeFromFormat($format);
			$this->fwPt = $pf[0];
			$this->fhPt = $pf[1];
		} else {
			if (isset($format['MediaBox'])) {
				$this->pagedim = WMS_TCPDF_STATIC::setPageBoxes($this->page, 'MediaBox', $format['MediaBox']['llx'], $format['MediaBox']['lly'], $format['MediaBox']['urx'], $format['MediaBox']['ury'], false, $this->k, $this->pagedim);
				...
				if (isset($format[0]) AND is_numeric($format[0]) AND isset($format[1]) AND is_numeric($format[1])) {
					...
					$pf = WMS_TCPDF_STATIC::getPageSizeFromFormat($format['format']);
				}
				$this->fwPt = $pf[0];
				$this->fhPt = $pf[1];
				$this->pagedim = WMS_TCPDF_STATIC::setPageBoxes($this->page, 'MediaBox', 0, 0, $this->fwPt, $this->fhPt, true, $this->k, $this->pagedim);
			}
			if (isset($format['CropBox'])) {
				$this->pagedim = WMS_TCPDF_STATIC::setPageBoxes($this->page, 'CropBox', $format['CropBox']['llx'], $format['CropBox']['lly'], $format['CropBox']['urx'], $format['CropBox']['ury'], false, $this->k, $this->pagedim);
			}
			if (isset($format['BleedBox'])) {
				$this->pagedim = WMS_TCPDF_STATIC::setPageBoxes($this->page, 'BleedBox', $format['BleedBox']['llx'], $format['BleedBox']['lly'], $format['BleedBox']['urx'], $format['BleedBox']['ury'], false, $this->k, $this->pagedim);
			}
			if (isset($format['TrimBox'])) {
				$this->pagedim = WMS_TCPDF_STATIC::setPageBoxes($this->page, 'TrimBox', $format['TrimBox']['llx'], $format['TrimBox']['lly'], $format['TrimBox']['urx'], $format['TrimBox']['ury'], false, $this->k, $this->pagedim);
			}
			if (isset($format['ArtBox'])) {
				$this->pagedim = WMS_TCPDF_STATIC::setPageBoxes($this->page, 'ArtBox', $format['ArtBox']['llx'], $format['ArtBox']['lly'], $format['ArtBox']['urx'], $format['ArtBox']['ury'], false, $this->k, $this->pagedim);
			}
	...
	public function setPageOrientation($orientation, $autopagebreak='', $bottommargin='') {
		...
		if (!isset($this->pagedim[$this->page]['CropBox'])) {
			$this->pagedim = WMS_TCPDF_STATIC::setPageBoxes($this->page, 'CropBox', $this->pagedim[$this->page]['MediaBox']['llx'], $this->pagedim[$this->page]['MediaBox']['lly'], $this->pagedim[$this->page]['MediaBox']['urx'], $this->pagedim[$this->page]['MediaBox']['ury'], true, $this->k, $this->pagedim);
		}
		if (!isset($this->pagedim[$this->page]['BleedBox'])) {
			$this->pagedim = WMS_TCPDF_STATIC::setPageBoxes($this->page, 'BleedBox', $this->pagedim[$this->page]['CropBox']['llx'], $this->pagedim[$this->page]['CropBox']['lly'], $this->pagedim[$this->page]['CropBox']['urx'], $this->pagedim[$this->page]['CropBox']['ury'], true, $this->k, $this->pagedim);
		}
		if (!isset($this->pagedim[$this->page]['TrimBox'])) {
			$this->pagedim = WMS_TCPDF_STATIC::setPageBoxes($this->page, 'TrimBox', $this->pagedim[$this->page]['CropBox']['llx'], $this->pagedim[$this->page]['CropBox']['lly'], $this->pagedim[$this->page]['CropBox']['urx'], $this->pagedim[$this->page]['CropBox']['ury'], true, $this->k, $this->pagedim);
		}
		if (!isset($this->pagedim[$this->page]['ArtBox'])) {
			$this->pagedim = WMS_TCPDF_STATIC::setPageBoxes($this->page, 'ArtBox', $this->pagedim[$this->page]['CropBox']['llx'], $this->pagedim[$this->page]['CropBox']['lly'], $this->pagedim[$this->page]['CropBox']['urx'], $this->pagedim[$this->page]['CropBox']['ury'], true, $this->k, $this->pagedim);
		}
		...
		if ((abs($this->pagedim[$this->page]['MediaBox']['urx'] - $this->hPt) < $this->feps) AND (abs($this->pagedim[$this->page]['MediaBox']['ury'] - $this->wPt) < $this->feps)){
			$this->pagedim = WMS_TCPDF_STATIC::swapPageBoxCoordinates($this->page, $this->pagedim);
		}
		$this->w = ($this->wPt / $this->k);
		$this->h = ($this->hPt / $this->k);
		if (WMS_TCPDF_STATIC::empty_string($autopagebreak)) {
		...
		if (WMS_TCPDF_STATIC::empty_string($bottommargin)) {
	...
	public function SetDisplayMode($zoom, $layout='SinglePage', $mode='UseNone') {
		...
		$this->LayoutMode = WMS_TCPDF_STATIC::getPageLayoutMode($layout);
		$this->PageMode = WMS_TCPDF_STATIC::getPageMode($mode);
	}
	...
	public function Error($msg) {
		$this->_destroy(true);
		if (defined('WMS_K_TCPDF_THROW_EXCEPTION_ERROR') AND !WMS_K_TCPDF_THROW_EXCEPTION_ERROR) {
	...
	public function Close() {
		...
		if ($this->tcpdflink) {
			...
			$font = defined('WMS_PDF_FONT_NAME_MAIN')?WMS_PDF_FONT_NAME_MAIN:'helvetica';
			$this->SetFont($font, '', 1);
	...
	public function Header() {
		if ($this->header_xobjid === false) {
			...
			if (($headerdata['logo']) AND ($headerdata['logo'] != WMS_K_BLANK_IMAGE)) {
				$imgtype = WMS_TCPDF_IMAGES::getImageFileType(WMS_K_PATH_IMAGES . $headerdata['logo']);
				if (($imgtype == 'eps') OR ($imgtype == 'ai')) {
					$this->ImageEps(WMS_K_PATH_IMAGES.$headerdata['logo'], '', '', $headerdata['logo_width']);
				} elseif ($imgtype == 'svg') {
					$this->ImageSVG(WMS_K_PATH_IMAGES.$headerdata['logo'], '', '', $headerdata['logo_width']);
				} else {
					$this->Image(WMS_K_PATH_IMAGES.$headerdata['logo'], '', '', $headerdata['logo_width']);
				}
	...
	protected function setTableHeader() {
		...
		if (!WMS_TCPDF_STATIC::empty_string($this->thead) AND (!$this->inthead)) {
	...
	public function setSpotColor($type, $name, $tint=100) {
		$spotcolor = WMS_TCPDF_COLORS::getSpotColor($name, $this->spot_colors);
		if ($spotcolor === false) {
	...
	public function GetStringWidth($s, $fontname='', $fontstyle='', $fontsize=0, $getarray=false) {
		return $this->GetArrStringWidth(WMS_TCPDF_FONTS::utf8Bidi(WMS_TCPDF_FONTS::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont), $s, $this->tmprtl, $this->isunicode, $this->CurrentFont), $fontname, $fontstyle, $fontsize, $getarray);
	}

	public function GetArrStringWidth($sa, $fontname='', $fontstyle='', $fontsize=0, $getarray=false) {
		if (!WMS_TCPDF_STATIC::empty_string($fontname)) {
		...
		if ($this->isunicode AND (!$this->isUnicodeFont())) {
			$sa = WMS_TCPDF_FONTS::UTF8ArrToLatin1Arr($sa);
		}
		$w = 0; // total width
	...
	public function GetNumChars($s) {
		if ($this->isUnicodeFont()) {
			return count(WMS_TCPDF_FONTS::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont));
		}
		return strlen($s);
	}

	protected function getFontsList() {
		if (($fontsdir = opendir(WMS_TCPDF_FONTS::_getfontpath())) !== false) {
	...
	public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		if (WMS_TCPDF_STATIC::empty_string($family)) {
			if (!WMS_TCPDF_STATIC::empty_string($this->FontFamily)) {
		...
		$fontdir = false;
		if (!WMS_TCPDF_STATIC::empty_string($fontfile)) {
			$fontdir = dirname($fontfile);
			if (WMS_TCPDF_STATIC::empty_string($fontdir) OR ($fontdir == '.')) {
				$fontdir = '';
			} else {
				$fontdir .= '/';
			}
		...
		$missing_style = false;
		if (WMS_TCPDF_STATIC::empty_string($fontfile) OR (!@WMS_TCPDF_STATIC::file_exists($fontfile))) {
			$tmp_fontfile = str_replace(' ', '', $family).strtolower($style).'.php';
			$fontfile = WMS_TCPDF_FONTS::getFontFullPath($tmp_fontfile, $fontdir);
			if (WMS_TCPDF_STATIC::empty_string($fontfile)) {
				$missing_style = true;
		...
		if (!WMS_TCPDF_STATIC::empty_string($fontfile) AND (@WMS_TCPDF_STATIC::file_exists($fontfile))) {
			include($fontfile);
		} else {
		...
		if (!isset($file) OR WMS_TCPDF_STATIC::empty_string($file)) {
			$file = '';
		}
		if (!isset($enc) OR WMS_TCPDF_STATIC::empty_string($enc)) {
			$enc = '';
		}
		if (!isset($cidinfo) OR WMS_TCPDF_STATIC::empty_string($cidinfo)) {
			$cidinfo = array('Registry'=>'Adobe', 'Ordering'=>'Identity', 'Supplement'=>0);
			$cidinfo['uni2cid'] = array();
		}
		if (!isset($ctg) OR WMS_TCPDF_STATIC::empty_string($ctg)) {
			$ctg = '';
		}
		if (!isset($desc) OR WMS_TCPDF_STATIC::empty_string($desc)) {
			$desc = array();
		}
		if (!isset($up) OR WMS_TCPDF_STATIC::empty_string($up)) {
			$up = -100;
		}
		if (!isset($ut) OR WMS_TCPDF_STATIC::empty_string($ut)) {
			$ut = 50;
		}
		if (!isset($cw) OR WMS_TCPDF_STATIC::empty_string($cw)) {
			$cw = array();
		}
		if (!isset($dw) OR WMS_TCPDF_STATIC::empty_string($dw)) {
		...
		if (!WMS_TCPDF_STATIC::empty_string($file)) {
	...
	public function isCharDefined($char, $font='', $style='') {
		if (is_string($char)) {
			$char = WMS_TCPDF_FONTS::UTF8StringToArray($char, $this->isunicode, $this->CurrentFont);
			$char = $char[0];
		}
		if (WMS_TCPDF_STATIC::empty_string($font)) {
			if (WMS_TCPDF_STATIC::empty_string($style)) {
	...
	public function replaceMissingChars($text, $font='', $style='', $subs=array()) {
		...
		$fontdata = $this->AddFont($font, $style);
		$fontinfo = $this->getFontBuffer($fontdata['fontkey']);
		$uniarr = WMS_TCPDF_FONTS::UTF8StringToArray($text, $this->isunicode, $this->CurrentFont);
		...
		return WMS_TCPDF_FONTS::UniArrSubString(WMS_TCPDF_FONTS::UTF8ArrayToUniArray($uniarr, $this->isunicode));
	}
	...
	public function Annotation($x, $y, $w, $h, $text, $opt=array('Subtype'=>'Text'), $spaces=0) {
		...
		if (!$this->pdfa_mode || ($this->pdfa_mode && $this->pdfa_version == 3)) {
			if ((($opt['Subtype'] == 'FileAttachment') OR ($opt['Subtype'] == 'Sound')) AND (!WMS_TCPDF_STATIC::empty_string($opt['FS']))
				AND (@WMS_TCPDF_STATIC::file_exists($opt['FS']) OR WMS_TCPDF_STATIC::isValidURL($opt['FS']))
				AND (!isset($this->embeddedfiles[basename($opt['FS'])]))) {
				$this->embeddedfiles[basename($opt['FS'])] = array('f' => ++$this->n, 'n' => ++$this->n, 'file' => $opt['FS']);
			}
		}
		if (isset($opt['mk']['i']) AND @WMS_TCPDF_STATIC::file_exists($opt['mk']['i'])) {
			$this->Image($opt['mk']['i'], '', '', 10, 10, '', '', '', false, 300, '', false, false, 0, false, true);
		}
		if (isset($opt['mk']['ri']) AND @WMS_TCPDF_STATIC::file_exists($opt['mk']['ri'])) {
			$this->Image($opt['mk']['ri'], '', '', 0, 0, '', '', '', false, 300, '', false, false, 0, false, true);
		}
		if (isset($opt['mk']['ix']) AND @WMS_TCPDF_STATIC::file_exists($opt['mk']['ix'])) {
	...
	protected function _putEmbeddedFiles() {
		...
		foreach ($this->embeddedfiles as $filename => $filedata) {
			$data = WMS_TCPDF_STATIC::fileGetContents($filedata['file']);
			if ($data !== FALSE) {
				$rawsize = strlen($data);
				if ($rawsize > 0) {
	...
	protected function checkPageBreak($h=0, $y='', $addpage=true) {
		if (WMS_TCPDF_STATIC::empty_string($y)) {
			$y = $this->y;
		}
	...
	protected function getCellCode($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
		$txt = str_replace(WMS_TCPDF_FONTS::unichr(160, $this->isunicode), ' ', $txt);
		$prev_cell_margin = $this->cell_margin;
		$prev_cell_padding = $this->cell_padding;
		$txt = WMS_TCPDF_STATIC::removeSHY($txt, $this->isunicode);
		...
		if (WMS_TCPDF_STATIC::empty_string($w) OR ($w <= 0)) {
			if ($this->rtl) {
				$w = $x - $this->lMargin;
			} else {
		...
		if ($txt != '') {
			...
			if ($this->isunicode) {
				if (($this->CurrentFont['type'] == 'core') OR ($this->CurrentFont['type'] == 'TrueType') OR ($this->CurrentFont['type'] == 'Type1')) {
					$txt2 = WMS_TCPDF_FONTS::UTF8ToLatin1($txt2, $this->isunicode, $this->CurrentFont);
				} else {
					$unicode = WMS_TCPDF_FONTS::UTF8StringToArray($txt, $this->isunicode, $this->CurrentFont); // array of UTF-8 unicode values
					$unicode = WMS_TCPDF_FONTS::utf8Bidi($unicode, '', $this->tmprtl, $this->isunicode, $this->CurrentFont);
					if (defined('WMS_K_THAI_TOPCHARS') AND (WMS_K_THAI_TOPCHARS == true)) {
						...
						$unicode = $output;
						$this->setFontSubBuffer($this->CurrentFont['fontkey'], 'subsetchars', $this->CurrentFont['subsetchars']);
					} // end of WMS_K_THAI_TOPCHARS
					$txt2 = WMS_TCPDF_FONTS::arrUTF8ToUTF16BE($unicode, false);
				}
			}
			$txt2 = WMS_TCPDF_STATIC::_escape($txt2);
			...
			if (isset($uniblock)) {
				...
				foreach ($uniblock as $uk => $uniarr) {
					if (($uk % 2) == 0) {
						...
						$topchr = WMS_TCPDF_FONTS::arrUTF8ToUTF16BE($uniarr, false);
						$topchr = WMS_TCPDF_STATIC::_escape($topchr);
	...
	public function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0, $valign='T', $fitcell=false) {
		...
		$this->setCellMargins(0, 0, 0, 0);
		if (WMS_TCPDF_STATIC::empty_string($this->lasth) OR $reseth) {
			$this->resetLastH();
		}
		if (!WMS_TCPDF_STATIC::empty_string($y)) {
		...
		if (!WMS_TCPDF_STATIC::empty_string($x)) {
			$this->SetX($x);
		} else {
			$x = $this->GetX();
		}
		...
		$border_start = WMS_TCPDF_STATIC::getBorderMode($border, $position='start', $this->opencell);
		$border_end = WMS_TCPDF_STATIC::getBorderMode($border, $position='end', $this->opencell);
		$border_middle = WMS_TCPDF_STATIC::getBorderMode($border, $position='middle', $this->opencell);
	...
	public function getNumLines($txt, $w=0, $reseth=false, $autopadding=true, $cellpadding='', $border=0) {
		...
		$lines = 1;
		$sum = 0;
		$chars = WMS_TCPDF_FONTS::utf8Bidi(WMS_TCPDF_FONTS::UTF8StringToArray($txt, $this->isunicode, $this->CurrentFont), $txt, $this->tmprtl, $this->isunicode, $this->CurrentFont);
		...
		for ($i = 0; $i < $length; ++$i) {
			...
			if (($c != 160)
					AND (($c == 173)
						OR preg_match($this->re_spaces, WMS_TCPDF_FONTS::unichr($c, $this->isunicode))
						OR (($c == 45)
							AND ($i > 0) AND ($i < ($length - 1))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], WMS_TCPDF_FONTS::unichr($chars[($i - 1)], $this->isunicode))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], WMS_TCPDF_FONTS::unichr($chars[($i + 1)], $this->isunicode))
						)
					)
				) {
	...
	public function Write($h, $txt, $link='', $fill=false, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0, $wadj=0, $margin='') {
		...
		$s = str_replace("\r", '', $txt);
		if (preg_match(WMS_TCPDF_FONT_DATA::$uni_RE_PATTERN_ARABIC, $s)) {
			$arabic = true;
		} else {
			$arabic = false;
		}
		if ($arabic OR ($this->tmprtl == 'R') OR preg_match(WMS_TCPDF_FONT_DATA::$uni_RE_PATTERN_RTL, $s)) {
			$rtlmode = true;
		} else {
			$rtlmode = false;
		}
		$chrwidth = $this->GetCharWidth(46); // dot character
		$chars = WMS_TCPDF_FONTS::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont);
		$chrw = $this->GetArrStringWidth($chars, '', '', 0, true);
		array_walk($chrw, array($this, 'getRawCharWidth'));
		$maxchwidth = max($chrw);
		$uchars = WMS_TCPDF_FONTS::UTF8ArrayToUniArray($chars, $this->isunicode);
		$nb = count($chars);
		$shy_replacement = 45;
		$shy_replacement_char = WMS_TCPDF_FONTS::unichr($shy_replacement, $this->isunicode);
		...
		while ($i < $nb) {
			...
			if ($c == 10) { // 10 = "\n" = new line
				...
				$tmpstr = WMS_TCPDF_FONTS::UniArrSubString($uchars, $j, $i);
				if ($firstline) {
					$startx = $this->x;
					$tmparr = array_slice($chars, $j, ($i - $j));
					if ($rtlmode) {
						$tmparr = WMS_TCPDF_FONTS::utf8Bidi($tmparr, $tmpstr, $this->tmprtl, $this->isunicode, $this->CurrentFont);
					}
				...
				if ($firstline) {
					...
					return (WMS_TCPDF_FONTS::UniArrSubString($uchars, $i));
				}
				++$nl;
				$j = $i + 1;
				$l = 0;
				...
				if (($c != 160)
					AND (($c == 173)
						...
						OR (($c == 45)
							AND ($i < ($nb - 1))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], WMS_TCPDF_FONTS::unichr($pc, $this->isunicode))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], WMS_TCPDF_FONTS::unichr($chars[($i + 1)], $this->isunicode))
						)
					)
				) {
				...
				if ($this->isUnicodeFont() AND ($arabic)) {
					$l = $this->GetArrStringWidth(WMS_TCPDF_FONTS::utf8Bidi(array_slice($chars, $j, ($i - $j)), '', $this->tmprtl, $this->isunicode, $this->CurrentFont));
				} else {
				...
				if (($l > $wmax) OR (($c == 173) AND (($l + $tmp_shy_replacement_width) >= $wmax))) {
					...
					if ($sep == -1) {
						if (($this->rtl AND ($this->x <= ($this->w - $this->rMargin - $this->cell_padding['R'] - $margin['R'] - $chrwidth)))
							...
							$linebreak = true;
							if ($firstline) {
								return (WMS_TCPDF_FONTS::UniArrSubString($uchars, $j));
							}
						} else {
							$tmpstr = WMS_TCPDF_FONTS::UniArrSubString($uchars, $j, $i);
							if ($firstline) {
						...
						$strrest = WMS_TCPDF_FONTS::UniArrSubString($uchars, ($sep + $endspace));
						$nextstr = WMS_TCPDF_STATIC::pregSplit('/' . $this->re_space['p'] . '/', $this->re_space['m'], $this->stringTrim($strrest));
						if (isset($nextstr[0]) AND ($this->GetStringWidth($nextstr[0]) > $pw)) {
							...
							$tmpstr = WMS_TCPDF_FONTS::UniArrSubString($uchars, $j, ($sep + $endspace));
							if ($firstline) {
								$startx = $this->x;
							...
							if ($firstline) {
								...
								$this->cell_padding = $tmpcellpadding;
								return (WMS_TCPDF_FONTS::UniArrSubString($uchars, ($sep + $endspace)));
							}
							$i = $sep;
							$sep = -1;
							$shy = false;
		...
		if ($l > 0) {
			...
			$tmpstr = WMS_TCPDF_FONTS::UniArrSubString($uchars, $j, $nb);
			if ($firstline) {
				$startx = $this->x;
			...
			if ($firstline) {
				$this->cell_padding = $tmpcellpadding;
				return (WMS_TCPDF_FONTS::UniArrSubString($uchars, $nb));
			}
			++$nl;
		}
		if ($firstline) {
	...
	public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if ($file[0] === '@') {
			...
			if (!@WMS_TCPDF_STATIC::file_exists($file)) {
				return false;
			}
			if (($imsize = @getimagesize($file)) === FALSE) {
				if (in_array($file, $this->imagekeys)) {
					...
					$imgdata = WMS_TCPDF_STATIC::fileGetContents($file);
				}
			}
		}
		if (!empty($imgdata)) {
			$original_file = $file;
			$file = WMS_TCPDF_STATIC::getObjFilename('img', $this->file_id);
			$fp = WMS_TCPDF_STATIC::fopenLocal($file, 'w');
			if (!$fp) {
		...
		if (in_array($file, $this->imagekeys)) {
			...
			$tempfile_plain = WMS_K_PATH_CACHE.'__tcpdf_'.$this->file_id.'_imgmask_plain_'.$filehash;
			$tempfile_alpha = WMS_K_PATH_CACHE.'__tcpdf_'.$this->file_id.'_imgmask_alpha_'.$filehash;
		...
		if ($newimage) {
			...
			if ($type == '') {
				$type = WMS_TCPDF_IMAGES::getImageFileType($file, $imsize);
			} elseif ($type == 'jpg') {
				$type = 'jpeg';
			}
			$mqr = WMS_TCPDF_STATIC::get_mqr();
			WMS_TCPDF_STATIC::set_mqr(false);
			$mtd = '_parse'.$type;
			$gdfunction = 'imagecreatefrom'.$type;
			$info = false;
			if ((method_exists('WMS_TCPDF_IMAGES', $mtd)) AND (!($resize AND (function_exists($gdfunction) OR extension_loaded('imagick'))))) {
				$info = WMS_TCPDF_IMAGES::$mtd($file);
					...
					if ($img !== false) {
						if ($resize) {
							...
							if (($type == 'gif') OR ($type == 'png')) {
								$imgr = WMS_TCPDF_IMAGES::setGDImageTransparency($imgr, $img);
							}
							imagecopyresampled($imgr, $img, 0, 0, 0, 0, $neww, $newh, $pixw, $pixh);
							$img = $imgr;
						}
						if (($type == 'gif') OR ($type == 'png')) {
							$info = WMS_TCPDF_IMAGES::_toPNG($img, WMS_TCPDF_STATIC::getObjFilename('img', $this->file_id));
						} else {
							$info = WMS_TCPDF_IMAGES::_toJPEG($img, $this->jpeg_quality, WMS_TCPDF_STATIC::getObjFilename('img', $this->file_id));
						}
					}
				} catch(\Exception $e) {
					$info = false;
				...
				try {
					...
					if ($type == 'svg') {
						if ($file[0] === '@') {
							...
							$svgimg = WMS_TCPDF_STATIC::fileGetContents($file);
						}
						if ($svgimg !== FALSE) {
							$regs = array();
					...
					$tempname = WMS_TCPDF_STATIC::getObjFilename('img', $this->file_id);
					$img->writeImage($tempname);
					$info = WMS_TCPDF_IMAGES::_parsejpeg($tempname);
					unlink($tempname);
					$img->destroy();
				} catch(\Exception $e) {
					$info = false;
	...
	protected function ImagePngAlpha($file, $x, $y, $wpx, $hpx, $w, $h, $type, $link, $align, $resize, $dpi, $palign, $filehash='') {
		...
		if (($parsed === false) AND extension_loaded('imagick')) {
			try {
				$img = new Imagick();
				$img->readImage($file);
				$imga = WMS_TCPDF_STATIC::objclone($img);
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			$byterange_string_len = strlen(WMS_TCPDF_STATIC::$byterange_string);
			$byte_range = array();
			$byte_range[0] = 0;
			$byte_range[1] = strpos($pdfdoc, WMS_TCPDF_STATIC::$byterange_string) + $byterange_string_len + 10;
			$byte_range[2] = $byte_range[1] + $this->signature_max_length + 2;
			$byte_range[3] = strlen($pdfdoc) - $byte_range[2];
			$pdfdoc = substr($pdfdoc, 0, $byte_range[1]).substr($pdfdoc, $byte_range[2]);
			$byterange = sprintf('/ByteRange[0 %u %u %u]', $byte_range[1], $byte_range[2], $byte_range[3]);
			$byterange .= str_repeat(' ', ($byterange_string_len - strlen($byterange)));
			$pdfdoc = str_replace(WMS_TCPDF_STATIC::$byterange_string, $byterange, $pdfdoc);
			$tempdoc = WMS_TCPDF_STATIC::getObjFilename('doc', $this->file_id);
			$f = WMS_TCPDF_STATIC::fopenLocal($tempdoc, 'wb');
			if (!$f) {
			...
			fclose($f);
			$tempsign = WMS_TCPDF_STATIC::getObjFilename('sig', $this->file_id);
		...
		switch($dest) {
			case 'I': {
				...
				if (php_sapi_name() != 'cli') {
					...
					WMS_TCPDF_STATIC::sendOutputData($this->getBuffer(), $this->bufferlen);
				} else {
					echo $this->getBuffer();
				}
				break;
			...
			case 'FI':
			case 'FD': {
				$f = WMS_TCPDF_STATIC::fopenLocal($name, 'wb');
				if (!$f) {
				...
				if ($dest == 'FI') {
					...
					header('Last-Modified: '.gmdate('D, d M Y H:i:s').' GMT');
					header('Content-Disposition: inline; filename="'.basename($name).'"');
					WMS_TCPDF_STATIC::sendOutputData(file_get_contents($name), filesize($name));
				} elseif ($dest == 'FD') {
					if (ob_get_contents()) {
						$this->Error('Some data has already been output, can\'t send PDF file');
					}
	...
	public function _destroy($destroyall=false, $preserve_objcopy=false) {
		...
		if ($destroyall AND !$preserve_objcopy) {
			...
			if ($handle = opendir(WMS_K_PATH_CACHE)) {
				while ( false !== ( $file_name = readdir( $handle ) ) ) {
					if (strpos($file_name, '__tcpdf_'.$this->file_id.'_') === 0) {
						unlink(WMS_K_PATH_CACHE.$file_name);
					}
				}
				closedir($handle);
			}
	...
	protected function getInternalPageNumberAliases($a= '') {
		...
		$u = '{'.$a.'}';
		$alias['u'][] = WMS_TCPDF_STATIC::_escape($u);
		if ($this->isunicode) {
			$alias['u'][] = WMS_TCPDF_STATIC::_escape(WMS_TCPDF_FONTS::UTF8ToLatin1($u, $this->isunicode, $this->CurrentFont));
			$alias['u'][] = WMS_TCPDF_STATIC::_escape(WMS_TCPDF_FONTS::utf8StrRev($u, false, $this->tmprtl, $this->isunicode, $this->CurrentFont));
			$alias['a'][] = WMS_TCPDF_STATIC::_escape(WMS_TCPDF_FONTS::UTF8ToLatin1($a, $this->isunicode, $this->CurrentFont));
			$alias['a'][] = WMS_TCPDF_STATIC::_escape(WMS_TCPDF_FONTS::utf8StrRev($a, false, $this->tmprtl, $this->isunicode, $this->CurrentFont));
		}
		$alias['a'][] = WMS_TCPDF_STATIC::_escape($a);
		return $alias;
	}

	protected function getAllInternalPageNumberAliases() {
		$basic_alias = array(WMS_TCPDF_STATIC::$alias_tot_pages, WMS_TCPDF_STATIC::$alias_num_page, WMS_TCPDF_STATIC::$alias_group_tot_pages, WMS_TCPDF_STATIC::$alias_group_num_page, WMS_TCPDF_STATIC::$alias_right_shift);
		$pnalias = array();
			...
			foreach ($alias as $a) {
				...
				if (($pos = strpos($page, $a)) !== false) {
					...
					if ($type == 'u') {
						...
						$shift = WMS_TCPDF_FONTS::UTF8ToUTF16BE($shift, false, $this->isunicode, $this->CurrentFont);
					} else {
	...
	protected function setPageBoxTypes($boxes) {
		...
		foreach ($boxes as $box) {
			if (in_array($box, WMS_TCPDF_STATIC::$pageboxes)) {
				$this->page_boxes[] = $box;
			}
		}
	}
	...
	protected function _putpages() {
		...
		$ptpa = WMS_TCPDF_STATIC::formatPageNumber(($this->starting_page_number + $num_pages - 1));
		$ptpu = WMS_TCPDF_FONTS::UTF8ToUTF16BE($ptpa, false, $this->isunicode, $this->CurrentFont);
		...
		for ($n = 1; $n <= $num_pages; ++$n) {
			...
			$pnpa = WMS_TCPDF_STATIC::formatPageNumber(($this->starting_page_number + $n - 1));
			$pnpu = WMS_TCPDF_FONTS::UTF8ToUTF16BE($pnpa, false, $this->isunicode, $this->CurrentFont);
			...
			if (!empty($this->pagegroups)) {
				if (isset($this->newpagegroup[$n])) {
					$pagegroupnum = 0;
					++$groupnum;
					$ptga = WMS_TCPDF_STATIC::formatPageNumber($this->pagegroups[$groupnum]);
					$ptgu = WMS_TCPDF_FONTS::UTF8ToUTF16BE($ptga, false, $this->isunicode, $this->CurrentFont);
					$ptg_num_chars = $this->GetNumChars($ptga);
				}
				++$pagegroupnum;
				$pnga = WMS_TCPDF_STATIC::formatPageNumber($pagegroupnum);
				$pngu = WMS_TCPDF_FONTS::UTF8ToUTF16BE($pnga, false, $this->isunicode, $this->CurrentFont);
				...
				list($temppage, $gdiff) = WMS_TCPDF_STATIC::replacePageNumAliases($temppage, $replace, $gdiff);
			}
			$replace = array();
			...
			list($temppage, $pdiff) = WMS_TCPDF_STATIC::replacePageNumAliases($temppage, $replace, $pdiff);
			...
			if (isset($this->PageAnnots[$n])) {
				foreach ($this->PageAnnots[$n] as $key => $pl) {
					...
					if (isset($pl['opt']['c']) AND (is_array($pl['opt']['c'])) AND !empty($pl['opt']['c'])) {
						$annots .= ' /C ' . WMS_TCPDF_COLORS::getColorStringFromArray($pl['opt']['c']);
					}
					...
					switch (strtolower($pl['opt']['subtype'])) {
						...
						case 'link': {
							if (is_string($pl['txt']) && !empty($pl['txt'])) {
								if ($pl['txt'][0] == '#') {
									$annots .= ' /A <</S /GoTo /D ' . WMS_TCPDF_STATIC::encodeNameObject(substr($pl['txt'], 1)) . '>>';
								} elseif ($pl['txt'][0] == '%') {
						...
						case 'widget': {
							...
							if (isset($pl['opt']['mk']) AND (is_array($pl['opt']['mk'])) AND !empty($pl['opt']['mk'])) {
								...
								if (isset($pl['opt']['mk']['bc']) AND (is_array($pl['opt']['mk']['bc']))) {
									$annots .= ' /BC ' . WMS_TCPDF_COLORS::getColorStringFromArray($pl['opt']['mk']['bc']);
								}
								if (isset($pl['opt']['mk']['bg']) AND (is_array($pl['opt']['mk']['bg']))) {
									$annots .= ' /BG ' . WMS_TCPDF_COLORS::getColorStringFromArray($pl['opt']['mk']['bg']);
								}
	...
	protected function _putfonts() {
		...
		foreach ($this->FontFiles as $file => $info) {
			$fontfile = WMS_TCPDF_FONTS::getFontFullPath($file, $info['fontdir']);
			if (!WMS_TCPDF_STATIC::empty_string($fontfile)) {
				...
				if ((!$compressed) AND (isset($info['length2']))) {
					...
					$font = WMS_TCPDF_FONTS::_getTrueTypeFontSubset($font, $subsetchars);
		...
		foreach ($this->fontkeys as $k) {
			...
			if ($type == 'core') {
				...
				if (!WMS_TCPDF_STATIC::empty_string($font['file'])) {
	...
	protected function _puttruetypeunicode($font) {
		...
		$out .= "\n".'endobj';
		$this->_out($out);
		$stream = WMS_TCPDF_FONT_DATA::$uni_identity_h;
		$this->_newobj();
		...
		$out .= "\n" . WMS_TCPDF_FONTS::_putfontwidths($font, 0);
		if (isset($font['ctg']) AND (!WMS_TCPDF_STATIC::empty_string($font['ctg']))) {
		...
		if (isset($font['ctg']) AND (!WMS_TCPDF_STATIC::empty_string($font['ctg']))) {
			...
			$fontfile = WMS_TCPDF_FONTS::getFontFullPath($ctgfile, $fontdir);
			if (WMS_TCPDF_STATIC::empty_string($fontfile)) {
	...
	protected function _putcidfont0($font) {
		...
		$out .= ' /FontDescriptor '.($this->n + 1).' 0 R';
		$out .= ' /DW '.$font['dw'];
		$out .= "\n" . WMS_TCPDF_FONTS::_putfontwidths($font, $cidoffset);
		$out .= ' >>';
		$out .= "\n".'endobj';
		$this->_out($out);
		$this->_newobj();
	...
	protected function _putinfo() {
		...
		if (!WMS_TCPDF_STATIC::empty_string($this->title)) {
			$out .= ' /Title '.$this->_textstring($this->title, $oid);
		}
		if (!WMS_TCPDF_STATIC::empty_string($this->author)) {
			$out .= ' /Author '.$this->_textstring($this->author, $oid);
		}
		if (!WMS_TCPDF_STATIC::empty_string($this->subject)) {
			$out .= ' /Subject '.$this->_textstring($this->subject, $oid);
		}
		if (!WMS_TCPDF_STATIC::empty_string($this->keywords)) {
			$out .= ' /Keywords '.$this->_textstring($this->keywords, $oid);
		}
		if (!WMS_TCPDF_STATIC::empty_string($this->creator)) {
			$out .= ' /Creator '.$this->_textstring($this->creator, $oid);
		}
		$this->isunicode = $prev_isunicode;
		$out .= ' /Producer '.$this->_textstring(WMS_TCPDF_STATIC::getTCPDFProducer(), $oid);
	...
	protected function _putXMP() {
		...
		$this->encrypted = false;
		$xmp = '<?xpacket begin="' . WMS_TCPDF_FONTS::unichr(0xfeff, $this->isunicode) . '" id="W5M0MpCehiHzreSzNTczkc9d"?>' . "\n";
		...
		$xmp .= "\t\t\t".'<dc:title>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Alt>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li xml:lang="x-default">' . WMS_TCPDF_STATIC::_escapeXML($this->title) . '</rdf:li>' . "\n";
		$xmp .= "\t\t\t\t".'</rdf:Alt>'."\n";
		$xmp .= "\t\t\t".'</dc:title>'."\n";
		$xmp .= "\t\t\t".'<dc:creator>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Seq>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li>' . WMS_TCPDF_STATIC::_escapeXML($this->author) . '</rdf:li>' . "\n";
		$xmp .= "\t\t\t\t".'</rdf:Seq>'."\n";
		$xmp .= "\t\t\t".'</dc:creator>'."\n";
		$xmp .= "\t\t\t".'<dc:description>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Alt>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li xml:lang="x-default">' . WMS_TCPDF_STATIC::_escapeXML($this->subject) . '</rdf:li>' . "\n";
		$xmp .= "\t\t\t\t".'</rdf:Alt>'."\n";
		$xmp .= "\t\t\t".'</dc:description>'."\n";
		$xmp .= "\t\t\t".'<dc:subject>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Bag>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li>' . WMS_TCPDF_STATIC::_escapeXML($this->keywords) . '</rdf:li>' . "\n";
		$xmp .= "\t\t\t\t".'</rdf:Bag>'."\n";
		$xmp .= "\t\t\t".'</dc:subject>'."\n";
		$xmp .= "\t\t".'</rdf:Description>'."\n";
		$dcdate = WMS_TCPDF_STATIC::getFormattedDate($this->doc_creation_timestamp);
		...
		$doccreationdate = WMS_TCPDF_STATIC::_escapeXML($doccreationdate);
		$dmdate = WMS_TCPDF_STATIC::getFormattedDate($this->doc_modification_timestamp);
		...
		$docmoddate = WMS_TCPDF_STATIC::_escapeXML($docmoddate);
		...
		$xmp .= "\t\t\t".'<pdf:Keywords>' . WMS_TCPDF_STATIC::_escapeXML($this->keywords) . '</pdf:Keywords>' . "\n";
		$xmp .= "\t\t\t".'<pdf:Producer>' . WMS_TCPDF_STATIC::_escapeXML(WMS_TCPDF_STATIC::getTCPDFProducer()) . '</pdf:Producer>' . "\n";
		$xmp .= "\t\t".'</rdf:Description>'."\n";
	...
	protected function _putcatalog() {
		...
		if (isset($this->LayoutMode) AND (!WMS_TCPDF_STATIC::empty_string($this->LayoutMode))) {
			$out .= ' /PageLayout /'.$this->LayoutMode;
		}
		if (isset($this->PageMode) AND (!WMS_TCPDF_STATIC::empty_string($this->PageMode))) {
	...
	protected function _putviewerpreferences() {
		...
		if (isset($vp['Duplex']) AND (!WMS_TCPDF_STATIC::empty_string($vp['Duplex']))) {
	...
	protected function _beginpage($orientation='', $format='') {
		...
		$this->state = 2;
		if (WMS_TCPDF_STATIC::empty_string($orientation)) {
			if (isset($this->CurOrientation)) {
		...
		if (WMS_TCPDF_STATIC::empty_string($format)) {
	...
	protected function _datastring($s, $n=0) {
		...
		return '(' . WMS_TCPDF_STATIC::_escape($s) . ')';
	}

	public function setDocCreationTimestamp($time) {
		if (is_string($time)) {
			$time = WMS_TCPDF_STATIC::getTimestamp($time);
		}
	...
	protected function _datestring($n=0, $timestamp=0) {
		...
		return $this->_datastring('D:' . WMS_TCPDF_STATIC::getFormattedDate($timestamp), $n);
	}

	protected function _textstring($s, $n=0) {
		if ($this->isunicode) {
			$s = WMS_TCPDF_FONTS::UTF8ToUTF16BE($s, true, $this->isunicode, $this->CurrentFont);
		}
	...
	protected function _objectkey($n) {
		...
		$objkey = substr(WMS_TCPDF_STATIC::_md5_16($objkey), 0, (($this->encryptdata['Length'] / 8) + 5));
	...
	protected function _encrypt_data($n, $s) {
		...
		switch ($this->encryptdata['mode']) {
			case 0:   // RC4-40
			case 1: { // RC4-128
				$s = WMS_TCPDF_STATIC::_RC4($this->_objectkey($n), $s, $this->last_enc_key, $this->last_enc_key_c);
				break;
			}
			case 2: { // AES-128
				$s = WMS_TCPDF_STATIC::_AES($this->_objectkey($n), $s);
				break;
			}
			case 3: { // AES-256
				$s = WMS_TCPDF_STATIC::_AES($this->encryptdata['key'], $s);
				break;
			}
		}
		return $s;
	...
	protected function _putencryption() {
		...
		if ($this->encryptdata['pubkey']) {
			...
			if ($this->encryptdata['V'] == 5) { // AES-256
				$out .= ' 5';
				$out .= ' /OE (' . WMS_TCPDF_STATIC::_escape($this->encryptdata['OE']) . ')';
				$out .= ' /UE (' . WMS_TCPDF_STATIC::_escape($this->encryptdata['UE']) . ')';
				$out .= ' /Perms (' . WMS_TCPDF_STATIC::_escape($this->encryptdata['perms']) . ')';
			...
			$out .= ' /O (' . WMS_TCPDF_STATIC::_escape($this->encryptdata['O']) . ')';
			$out .= ' /U (' . WMS_TCPDF_STATIC::_escape($this->encryptdata['U']) . ')';
	...
	protected function _Uvalue() {
		if ($this->encryptdata['mode'] == 0) { // RC4-40
			return WMS_TCPDF_STATIC::_RC4($this->encryptdata['key'], WMS_TCPDF_STATIC::$enc_padding, $this->last_enc_key, $this->last_enc_key_c);
		} elseif ($this->encryptdata['mode'] < 3) { // RC4-128, AES-128
			$tmp = WMS_TCPDF_STATIC::_md5_16(WMS_TCPDF_STATIC::$enc_padding . $this->encryptdata['fileid']);
			$enc = WMS_TCPDF_STATIC::_RC4($this->encryptdata['key'], $tmp, $this->last_enc_key, $this->last_enc_key_c);
			$len = strlen($tmp);
			for ($i = 1; $i <= 19; ++$i) {
				$ek = '';
				for ($j = 0; $j < $len; ++$j) {
				...
				$enc = WMS_TCPDF_STATIC::_RC4($ek, $enc, $this->last_enc_key, $this->last_enc_key_c);
			}
			$enc .= str_repeat("\x00", 16);
			return substr($enc, 0, 32);
		} elseif ($this->encryptdata['mode'] == 3) { // AES-256
			$seed = WMS_TCPDF_STATIC::_md5_16(WMS_TCPDF_STATIC::getRandomSeed());
	...
	protected function _UEvalue() {
		...
		return WMS_TCPDF_STATIC::_AESnopad($hashkey, $this->encryptdata['key']);
	}

	protected function _Ovalue() {
		if ($this->encryptdata['mode'] < 3) { // RC4-40, RC4-128, AES-128
			$tmp = WMS_TCPDF_STATIC::_md5_16($this->encryptdata['owner_password']);
			if ($this->encryptdata['mode'] > 0) {
				for ($i = 0; $i < 50; ++$i) {
					$tmp = WMS_TCPDF_STATIC::_md5_16($tmp);
				}
			}
			$owner_key = substr($tmp, 0, ($this->encryptdata['Length'] / 8));
			$enc = WMS_TCPDF_STATIC::_RC4($owner_key, $this->encryptdata['user_password'], $this->last_enc_key, $this->last_enc_key_c);
			if ($this->encryptdata['mode'] > 0) {
	...
	protected function _fixAES256Password($password) {
		...
		$psw_array = WMS_TCPDF_FONTS::utf8Bidi(WMS_TCPDF_FONTS::UTF8StringToArray($password, $this->isunicode, $this->CurrentFont), $password, $this->rtl, $this->isunicode, $this->CurrentFont);
		foreach ($psw_array as $c) {
			$psw .= WMS_TCPDF_FONTS::unichr($c, $this->isunicode);
		}
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { // standard mode
			if ($this->encryptdata['mode'] == 3) { // AES-256
				$this->encryptdata['key'] = substr(hash('sha256', WMS_TCPDF_STATIC::getRandomSeed(), true), 0, $keybytelen);
				...
				$perms = WMS_TCPDF_STATIC::getEncPermissionsString($this->encryptdata['protection']); // bytes 0-3
				...
				$this->encryptdata['perms'] = WMS_TCPDF_STATIC::_AESnopad($this->encryptdata['key'], $perms);
			} else { // RC4-40, RC4-128, AES-128
				$this->encryptdata['user_password'] = substr($this->encryptdata['user_password'] . WMS_TCPDF_STATIC::$enc_padding, 0, 32);
				$this->encryptdata['owner_password'] = substr($this->encryptdata['owner_password'] . WMS_TCPDF_STATIC::$enc_padding, 0, 32);
				$this->encryptdata['O'] = $this->_Ovalue();
				$permissions = WMS_TCPDF_STATIC::getEncPermissionsString($this->encryptdata['protection']);
				$tmp = WMS_TCPDF_STATIC::_md5_16($this->encryptdata['user_password'] . $this->encryptdata['O'] . $permissions . $this->encryptdata['fileid']);
				if ($this->encryptdata['mode'] > 0) {
					for ($i = 0; $i < 50; ++$i) {
						$tmp = WMS_TCPDF_STATIC::_md5_16(substr($tmp, 0, $keybytelen));
					}
				}
			...
			$seed = sha1(WMS_TCPDF_STATIC::getRandomSeed(), true);
			$recipient_bytes = '';
			foreach ($this->encryptdata['pubkeys'] as $pubkey) {
				if (isset($pubkey['p'])) {
					$pkprotection = WMS_TCPDF_STATIC::getUserPermissionCode($pubkey['p'], $this->encryptdata['mode']);
				} else {
					$pkprotection = $this->encryptdata['protection'];
				}
				$pkpermissions = WMS_TCPDF_STATIC::getEncPermissionsString($pkprotection);
				$envelope = $seed.$pkpermissions;
				$tempkeyfile = WMS_TCPDF_STATIC::getObjFilename('key', $this->file_id);
				$f = WMS_TCPDF_STATIC::fopenLocal($tempkeyfile, 'wb');
				if (!$f) {
				...
				fclose($f);
				$tempencfile = WMS_TCPDF_STATIC::getObjFilename('enc', $this->file_id);
	...
	public function SetProtection($permissions=array('print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high'), $user_pass='', $owner_pass=null, $mode=0, $pubkeys=null) {
		...
		$this->encryptdata['protection'] = WMS_TCPDF_STATIC::getUserPermissionCode($permissions, $mode);
		...
		if ($owner_pass === null) {
			$owner_pass = md5(WMS_TCPDF_STATIC::getRandomSeed());
		}
		...
		$this->encrypted = true;
		$this->encryptdata['fileid'] = WMS_TCPDF_STATIC::convertHexStringToString($this->file_id);
	...
	public function Rect($x, $y, $w, $h, $style='', $border_style=array(), $fill_color=array()) {
		...
		if (!empty($style)) {
			$op = WMS_TCPDF_STATIC::getPathPaintOperator($style);
	...
	public function Ellipse($x0, $y0, $rx, $ry='', $angle=0, $astart=0, $afinish=360, $style='', $line_style=array(), $fill_color=array(), $nc=2) {
		...
		if (WMS_TCPDF_STATIC::empty_string($ry) OR ($ry == 0)) {
			$ry = $rx;
		}
		if (!(false === strpos($style, 'F')) AND isset($fill_color)) {
			$this->SetFillColorArray($fill_color);
	...
	public function setDestination($name, $y=-1, $page='', $x=-1) {
		$name = WMS_TCPDF_STATIC::encodeNameObject($name);
		if (WMS_TCPDF_STATIC::empty_string($name)) {
			return false;
		}
		if ($y == -1) {
			$y = $this->GetY();
	...
	protected function _putbookmarks() {
		...
		foreach ($this->outlines as $i => $o) {
			...
			if (isset($o['u']) AND !empty($o['u'])) {
				if (is_string($o['u'])) {
					if ($o['u'][0] == '#') {
						$out .= ' /Dest /' . WMS_TCPDF_STATIC::encodeNameObject(substr($o['u'], 1));
					} elseif ($o['u'][0] == '%') {
	...
	protected function _addfield($type, $name, $x, $y, $w, $h, $prop) {
		...
		foreach($prop as $key => $val) {
			if (strcmp(substr($key, -5), 'Color') == 0) {
				$val = WMS_TCPDF_COLORS::_JScolor($val);
			} else {
				$val = "'".$val."'";
			}
	...
	public function TextField($name, $w, $h, $prop=array(), $opt=array(), $x='', $y='', $js=false) {
		...
		$popt = WMS_TCPDF_STATIC::getAnnotOptFromJSProp($prop, $this->spot_colors, $this->rtl);
	...
	public function RadioButton($name, $w, $prop=array(), $opt=array(), $onvalue='On', $checked=false, $x='', $y='', $js=false) {
		...
		if (WMS_TCPDF_STATIC::empty_string($onvalue)) {
			$onvalue = 'On';
		}
		if ($checked) {
			$defval = $onvalue;
	...
	protected function _putsignature() {
		...
		$out .= ' ' . WMS_TCPDF_STATIC::$byterange_string;
		...
		if (empty($this->signature_data['approval']) OR ($this->signature_data['approval'] != 'A')) {
			...
			if ($this->signature_data['cert_type'] > 0) {
				...
				$out .= ' /Type /TransformParams';
				$out .= ' /V /2.2';
				if (!WMS_TCPDF_STATIC::empty_string($this->ur['document'])) {
					$out .= ' /Document['.$this->ur['document'].']';
				}
				if (!WMS_TCPDF_STATIC::empty_string($this->ur['form'])) {
					$out .= ' /Form['.$this->ur['form'].']';
				}
				if (!WMS_TCPDF_STATIC::empty_string($this->ur['signature'])) {
					$out .= ' /Signature['.$this->ur['signature'].']';
				}
				if (!WMS_TCPDF_STATIC::empty_string($this->ur['annots'])) {
					$out .= ' /Annots['.$this->ur['annots'].']';
				}
				if (!WMS_TCPDF_STATIC::empty_string($this->ur['ef'])) {
					$out .= ' /EF['.$this->ur['ef'].']';
				}
				if (!WMS_TCPDF_STATIC::empty_string($this->ur['formex'])) {
		...
		if (isset($this->signature_data['info']['Name']) AND !WMS_TCPDF_STATIC::empty_string($this->signature_data['info']['Name'])) {
			$out .= ' /Name '.$this->_textstring($this->signature_data['info']['Name'], $sigobjid);
		}
		if (isset($this->signature_data['info']['Location']) AND !WMS_TCPDF_STATIC::empty_string($this->signature_data['info']['Location'])) {
			$out .= ' /Location '.$this->_textstring($this->signature_data['info']['Location'], $sigobjid);
		}
		if (isset($this->signature_data['info']['Reason']) AND !WMS_TCPDF_STATIC::empty_string($this->signature_data['info']['Reason'])) {
			$out .= ' /Reason '.$this->_textstring($this->signature_data['info']['Reason'], $sigobjid);
		}
		if (isset($this->signature_data['info']['ContactInfo']) AND !WMS_TCPDF_STATIC::empty_string($this->signature_data['info']['ContactInfo'])) {
	...
	public function getAliasRightShift() {
		$ref = '{' . WMS_TCPDF_STATIC::$alias_right_shift . '}{' . WMS_TCPDF_STATIC::$alias_tot_pages . '}{' . WMS_TCPDF_STATIC::$alias_num_page . '}';
		...
		$alias = WMS_TCPDF_STATIC::$alias_right_shift . $sdiff . '}';
		if ($this->isUnicodeFont()) {
			$alias = '{'.$alias;
		}
		return $alias;
	...
	public function getAliasNbPages() {
		if ($this->isUnicodeFont()) {
			return '{' . WMS_TCPDF_STATIC::$alias_tot_pages . '}';
		}
		return WMS_TCPDF_STATIC::$alias_tot_pages;
	}
	...
	public function getAliasNumPage() {
		if ($this->isUnicodeFont()) {
			return '{' . WMS_TCPDF_STATIC::$alias_num_page . '}';
		}
		return WMS_TCPDF_STATIC::$alias_num_page;
	}
	...
	public function getPageGroupAlias() {
		if ($this->isUnicodeFont()) {
			return '{' . WMS_TCPDF_STATIC::$alias_group_tot_pages . '}';
		}
		return WMS_TCPDF_STATIC::$alias_group_tot_pages;
	}
	...
	public function getPageNumGroupAlias() {
		if ($this->isUnicodeFont()) {
			return '{' . WMS_TCPDF_STATIC::$alias_group_num_page . '}';
		}
		return WMS_TCPDF_STATIC::$alias_group_num_page;
	}
	...
	public function getGroupPageNoFormatted() {
		return WMS_TCPDF_STATIC::formatPageNumber($this->getGroupPageNo());
	}

	public function PageNoFormatted() {
		return WMS_TCPDF_STATIC::formatPageNumber($this->PageNo());
	}
	...
	public function setOverprint($stroking=true, $nonstroking='', $mode=0) {
		...
		if (WMS_TCPDF_STATIC::empty_string($nonstroking)) {
		...
		foreach ($bars as $col) {
			switch ($col) {
				...
				default: { // SPECIFIC SPOT COLOR NAME
					$col_a = array(0,0,0,0,'None');
					$col_b = WMS_TCPDF_COLORS::getSpotColor($col, $this->spot_colors);
					if ($col_b === false) {
						$col_b = array(100,100,100,100,'All');
					}
					break;
	...
	public function ImageEps($file, $x='', $y='', $w=0, $h=0, $link='', $useBoundingBox=true, $align='', $palign='', $border=0, $fitonpage=false, $fixoutvals=false) {
		...
		if ($file[0] === '@') { // image from string
			...
			$data = WMS_TCPDF_STATIC::fileGetContents($file);
		}
		if ($data === FALSE) {
	...
	public function write1DBarcode($code, $type, $x='', $y='', $w='', $h='', $xres='', $style=array(), $align='') {
		if (WMS_TCPDF_STATIC::empty_string(trim($code))) {
			return;
		}
		...
		if ($style['stretch']) {
			...
			if (WMS_TCPDF_STATIC::empty_string($xres)) {
		...
		if ($style['text']) {
			if (isset($style['label']) AND !WMS_TCPDF_STATIC::empty_string($style['label'])) {
				$label = $style['label'];
			} else {
				$label = $code;
			}
	...
	public function fixHTMLCode($html, $default_css='', $tagvs='', $tidy_options='') {
		return WMS_TCPDF_STATIC::fixHTMLCode($html, $default_css, $tagvs, $tidy_options, $this->tagvspaces);
	}
	...
	protected function getCSSBorderStyle($cssborder) {
		...
		$border['color'] = WMS_TCPDF_COLORS::convertHTMLColorToDec($color, $this->spot_colors);
		return $border;
	}
	...
	public function getHTMLFontUnits($val, $refsize=12, $parent_size=12, $defaultunit='pt') {
		$refsize = WMS_TCPDF_FONTS::getFontRefSize($refsize);
		$parent_size = WMS_TCPDF_FONTS::getFontRefSize($parent_size, $refsize);
		switch ($val) {
			case 'xx-small': {
				$size = ($refsize - 4);
				break;
				...
				if (preg_match('/type[\s]*=[\s]*"text\/css"/', $link, $type)) {
					...
					if (empty($type) OR (isset($type[1]) AND (($type[1] == 'all') OR ($type[1] == 'print')))) {
						...
						if (preg_match('/href[\s]*=[\s]*"([^"]*)"/', $link, $type) > 0) {
							$cssdata = WMS_TCPDF_STATIC::fileGetContents(trim($type[1]));
							if (($cssdata !== FALSE) AND (strlen($cssdata) > 0)) {
								$css = array_merge($css, WMS_TCPDF_STATIC::extractCSSproperties($cssdata));
							}
						}
					}
				}
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					if (($dom[$key]['value'] == 'tr') AND ($dom[($dom[$key]['parent'])]['thead'] === true)) {
						if (WMS_TCPDF_STATIC::empty_string($dom[($dom[($dom[$key]['parent'])]['parent'])]['thead'])) {
					...
					if (($dom[$key]['value'] == 'table') AND (!WMS_TCPDF_STATIC::empty_string($dom[($dom[$key]['parent'])]['thead']))) {
					...
					if (!empty($css)) {
						list($dom[$key]['csssel'], $dom[$key]['cssdata']) = WMS_TCPDF_STATIC::getCSSdataArray($dom, $key, $css);
						$dom[$key]['attribute']['style'] = WMS_TCPDF_STATIC::getTagStyleFromCSSarray($dom[$key]['cssdata']);
					}
					if (isset($dom[$key]['attribute']['style']) AND !empty($dom[$key]['attribute']['style'])) {
						...
						if (isset($dom[$key]['style']['color']) AND (!WMS_TCPDF_STATIC::empty_string($dom[$key]['style']['color']))) {
							$dom[$key]['fgcolor'] = WMS_TCPDF_COLORS::convertHTMLColorToDec($dom[$key]['style']['color'], $this->spot_colors);
						} elseif ($dom[$key]['value'] == 'a') {
							$dom[$key]['fgcolor'] = $this->htmlLinkColorArray;
						}
						if (isset($dom[$key]['style']['background-color']) AND (!WMS_TCPDF_STATIC::empty_string($dom[$key]['style']['background-color']))) {
							$dom[$key]['bgcolor'] = WMS_TCPDF_COLORS::convertHTMLColorToDec($dom[$key]['style']['background-color'], $this->spot_colors);
						}
						if (isset($dom[$key]['style']['text-decoration'])) {
							$decors = explode(' ', strtolower($dom[$key]['style']['text-decoration']));
							foreach ($decors as $dec) {
								$dec = trim($dec);
								if (!WMS_TCPDF_STATIC::empty_string($dec)) {
									if ($dec[0] == 'u') {
										$dom[$key]['fontstyle'] .= 'U';
									} elseif ($dec[0] == 'l') {
										$dom[$key]['fontstyle'] .= 'D';
						...
						if (isset($dom[$key]['style']['border-color'])) {
							...
							if (isset($brd_colors[3])) {
								$dom[$key]['border']['L']['color'] = WMS_TCPDF_COLORS::convertHTMLColorToDec($brd_colors[3], $this->spot_colors);
							}
							if (isset($brd_colors[1])) {
								$dom[$key]['border']['R']['color'] = WMS_TCPDF_COLORS::convertHTMLColorToDec($brd_colors[1], $this->spot_colors);
							}
							if (isset($brd_colors[0])) {
								$dom[$key]['border']['T']['color'] = WMS_TCPDF_COLORS::convertHTMLColorToDec($brd_colors[0], $this->spot_colors);
							}
							if (isset($brd_colors[2])) {
								$dom[$key]['border']['B']['color'] = WMS_TCPDF_COLORS::convertHTMLColorToDec($brd_colors[2], $this->spot_colors);
							}
						}
						...
						foreach ($cellside as $bsk => $bsv) {
							...
							if (isset($dom[$key]['style']['border-'.$bsv.'-color'])) {
								$dom[$key]['border'][$bsk]['color'] = WMS_TCPDF_COLORS::convertHTMLColorToDec($dom[$key]['style']['border-' . $bsv . '-color'], $this->spot_colors);
							}
					...
					if ((($dom[$key]['value'] == 'ul') OR ($dom[$key]['value'] == 'ol') OR ($dom[$key]['value'] == 'dl'))
						AND (!isset($dom[$key]['align']) OR WMS_TCPDF_STATIC::empty_string($dom[$key]['align']) OR ($dom[$key]['align'] != 'J'))) {
						if ($this->rtl) {
							$dom[$key]['align'] = 'R';
						} else {
							$dom[$key]['align'] = 'L';
					...
					if (($dom[$key]['value'] == 'small') OR ($dom[$key]['value'] == 'sup') OR ($dom[$key]['value'] == 'sub')) {
						if (!isset($dom[$key]['attribute']['size']) AND !isset($dom[$key]['style']['font-size'])) {
							$dom[$key]['fontsize'] = $dom[$key]['fontsize'] * WMS_K_SMALL_RATIO;
						}
					}
					...
					if (isset($dom[$key]['attribute']['color']) AND (!WMS_TCPDF_STATIC::empty_string($dom[$key]['attribute']['color']))) {
						$dom[$key]['fgcolor'] = WMS_TCPDF_COLORS::convertHTMLColorToDec($dom[$key]['attribute']['color'], $this->spot_colors);
					} elseif (!isset($dom[$key]['style']['color']) AND ($dom[$key]['value'] == 'a')) {
						$dom[$key]['fgcolor'] = $this->htmlLinkColorArray;
					}
					if (isset($dom[$key]['attribute']['bgcolor']) AND (!WMS_TCPDF_STATIC::empty_string($dom[$key]['attribute']['bgcolor']))) {
						$dom[$key]['bgcolor'] = WMS_TCPDF_COLORS::convertHTMLColorToDec($dom[$key]['attribute']['bgcolor'], $this->spot_colors);
					}
					if (isset($dom[$key]['attribute']['strokecolor']) AND (!WMS_TCPDF_STATIC::empty_string($dom[$key]['attribute']['strokecolor']))) {
						$dom[$key]['strokecolor'] = WMS_TCPDF_COLORS::convertHTMLColorToDec($dom[$key]['attribute']['strokecolor'], $this->spot_colors);
					}
					...
					if (isset($dom[$key]['attribute']['align']) AND (!WMS_TCPDF_STATIC::empty_string($dom[$key]['attribute']['align'])) AND ($dom[$key]['value'] !== 'img')) {
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		while ($key < $maxel) {
			...
			if (($dom[$key]['value'] == 'tr') AND isset($dom[$key]['thead']) AND $dom[$key]['thead']) {
				if (isset($dom[$key]['parent']) AND isset($dom[$dom[$key]['parent']]['thead']) AND !WMS_TCPDF_STATIC::empty_string($dom[$dom[$key]['parent']]['thead'])) {
					$this->inthead = true;
					$this->writeHTML($this->thead, false, false, false, false, '');
			...
			if ($dom[$key]['tag'] OR ($key == 0)) {
				...
				if (WMS_TCPDF_STATIC::empty_string($lalign)) {
					$lalign = $align;
				}
			}
				...
				if (isset($startlinex)) {
					...
					if ((isset($plalign) AND ((($plalign == 'C') OR ($plalign == 'J') OR (($plalign == 'R') AND (!$this->rtl)) OR (($plalign == 'L') AND ($this->rtl)))))) {
						...
						if ($this->isRTLTextDir()) { // RTL
							$pos1 = WMS_TCPDF_STATIC::revstrpos($pmid, '[(');
							if ($pos1 > 0) {
								$pos1 = intval($pos1);
								if ($this->isUnicodeFont()) {
									$pos2 = intval(WMS_TCPDF_STATIC::revstrpos($pmid, '[(' . chr(0) . chr(32)));
									$spacelen = 2;
								} else {
									$pos2 = intval(WMS_TCPDF_STATIC::revstrpos($pmid, '[(' . chr(32)));
									$spacelen = 1;
								}
								if ($pos1 == $pos2) {
							...
							if ($pos1 > 0) {
								$pos1 = intval($pos1);
								if ($this->isUnicodeFont()) {
									$pos2 = intval(WMS_TCPDF_STATIC::revstrpos($pmid, chr(0) . chr(32) . ')]')) + 2;
									$spacelen = 2;
								} else {
									$pos2 = intval(WMS_TCPDF_STATIC::revstrpos($pmid, chr(32) . ')]')) + 1;
									$spacelen = 1;
								}
								if ($pos1 == $pos2) {
							...
							if (preg_match_all('/\[\(([^\)]*)\)\]/x', $pmidtemp, $lnstring, PREG_PATTERN_ORDER)) {
								...
								while (preg_match('/([0-9\.\+\-]*)[\s](Td|cm|m|l|c|re)[\s]/x', $pmid, $strpiece, PREG_OFFSET_CAPTURE, $offset) == 1) {
									...
									switch ($strpiece[2][0]) {
										...
										case 're': {
											if (!WMS_TCPDF_STATIC::empty_string($this->lispacer)) {
												$this->lispacer = '';
												break;
											}
			...
			if ($dom[$key]['tag']) {
				...
				if (!WMS_TCPDF_STATIC::empty_string($this->lispacer) AND ($this->lispacer != '^')) {
				...
				$this->htmlvspace = 0;
				$isRTLString = preg_match(WMS_TCPDF_FONT_DATA::$uni_RE_PATTERN_RTL, $dom[$key]['value']) || preg_match(WMS_TCPDF_FONT_DATA::$uni_RE_PATTERN_ARABIC, $dom[$key]['value']);
				...
				if (!isset($dom[$key]['trimmed_space']) OR !$dom[$key]['trimmed_space']) {
					...
					if (!empty($this->HREF) AND (isset($this->HREF['url']))) {
						...
						if (($strlinelen < $cwa) AND (isset($dom[($key + 1)])) AND ($dom[($key + 1)]['tag']) AND (!$dom[($key + 1)]['block'])) {
							...
							while ($write_block AND isset($dom[$nkey])) {
								if ($dom[$nkey]['tag']) {
									...
									$nextstr = WMS_TCPDF_STATIC::pregSplit('/' . $this->re_space['p'] . '+/', $this->re_space['m'], $dom[$nkey]['value']);
						...
						if (($wadj > 0) AND (($strlinelen + $wadj) >= $cwa)) {
							$wadj = 0;
							$nextstr = WMS_TCPDF_STATIC::pregSplit('/' . $this->re_space['p'] . '/', $this->re_space['m'], $dom[$key]['value']);
		...
		switch($tag['value']) {
			case 'table': {
				...
				if (!isset($dom[$key]['attribute']['nested']) OR ($dom[$key]['attribute']['nested'] != 'true')) {
					$this->htmlvspace = 0;
					if (!WMS_TCPDF_STATIC::empty_string($dom[$key]['thead'])) {
			...
			case 'img': {
				...
				if ($imgsrc[0] === '@') {
					...
					if (($imgsrc[0] === '/') AND !empty($_SERVER['DOCUMENT_ROOT']) AND ($_SERVER['DOCUMENT_ROOT'] != '/')) {
						...
						if (empty($testscrtype['query'])) {
							$imgsrc = str_replace(WMS_K_PATH_URL, WMS_K_PATH_MAIN, $imgsrc);
						} elseif (preg_match('|^https?://|', $imgsrc) !== 1) {
							$imgsrc = str_replace(WMS_K_PATH_MAIN, WMS_K_PATH_URL, $imgsrc);
						}
					}
					$type = WMS_TCPDF_IMAGES::getImageFileType($imgsrc);
				}
				if (!isset($tag['width'])) {
					$tag['width'] = 0;
				}
				...
				$xpos = $this->x;
				$imglink = '';
				if (isset($this->HREF['url']) AND !WMS_TCPDF_STATIC::empty_string($this->HREF['url'])) {
			...
			case 'li': {
				...
				if ($this->listordered[$this->listnum]) {
					if (isset($parent['attribute']['type']) AND !WMS_TCPDF_STATIC::empty_string($parent['attribute']['type'])) {
						$this->lispacer = $parent['attribute']['type'];
					} elseif (isset($parent['listtype']) AND !WMS_TCPDF_STATIC::empty_string($parent['listtype'])) {
						$this->lispacer = $parent['listtype'];
					} elseif (isset($this->lisymbol) AND !WMS_TCPDF_STATIC::empty_string($this->lisymbol)) {
			...
			case 'input': {
				if (isset($tag['attribute']['name']) AND !WMS_TCPDF_STATIC::empty_string($tag['attribute']['name'])) {
				...
				$prop = array();
				$opt = array();
				if (isset($tag['attribute']['readonly']) AND !WMS_TCPDF_STATIC::empty_string($tag['attribute']['readonly'])) {
					$prop['readonly'] = true;
				}
				if (isset($tag['attribute']['value']) AND !WMS_TCPDF_STATIC::empty_string($tag['attribute']['value'])) {
					$value = $tag['attribute']['value'];
				}
				if (isset($tag['attribute']['maxlength']) AND !WMS_TCPDF_STATIC::empty_string($tag['attribute']['maxlength'])) {
					$opt['maxlen'] = intval($tag['attribute']['maxlength']);
				}
				$h = $this->getCellHeight($this->FontSize);
				if (isset($tag['attribute']['size']) AND !WMS_TCPDF_STATIC::empty_string($tag['attribute']['size'])) {
				...
				switch ($tag['attribute']['type']) {
					...
					case 'image': {
						if (isset($tag['attribute']['src']) AND !WMS_TCPDF_STATIC::empty_string($tag['attribute']['src'])) {
			...
			case 'textarea': {
				...
				if (isset($tag['attribute']['cols']) AND !WMS_TCPDF_STATIC::empty_string($tag['attribute']['cols'])) {
					$w = intval($tag['attribute']['cols']) * $this->GetStringWidth(chr(32)) * 2;
				} else {
					$w = 40;
				}
				if (isset($tag['attribute']['rows']) AND !WMS_TCPDF_STATIC::empty_string($tag['attribute']['rows'])) {
			...
			case 'select': {
				...
				$w = 0;
				if (isset($tag['attribute']['opt']) AND !WMS_TCPDF_STATIC::empty_string($tag['attribute']['opt'])) {
			...
			case 'tcpdf': {
				if (defined('WMS_K_TCPDF_CALLS_IN_HTML') AND (WMS_K_TCPDF_CALLS_IN_HTML === true)) {
					if (isset($tag['attribute']['method'])) {
	...
	protected function drawHTMLTagBorder($tag, $xmax) {
		...
		if (isset($tag['border']) AND !empty($tag['border'])) {
			$border = $tag['border'];
			if (!WMS_TCPDF_STATIC::empty_string($this->thead) AND (!$this->inthead)) {
				$border = WMS_TCPDF_STATIC::getBorderMode($border, $position='middle', $this->opencell);
			}
		}
	...
	protected function putHtmlListBullet($listdepth, $listtype='', $size=10) {
		...
		switch ($listtype) {
			...
			case 'i':
			case 'lower-roman': {
				$textitem = strtolower(WMS_TCPDF_STATIC::intToRoman($this->listcount[$this->listnum]));
				break;
			}
			case 'I':
			case 'upper-roman': {
				$textitem = WMS_TCPDF_STATIC::intToRoman($this->listcount[$this->listnum]);
				break;
			}
			case 'a':
			case 'lower-alpha':
			...
			case 'lower-greek': {
				$textitem = WMS_TCPDF_FONTS::unichr((945 + $this->listcount[$this->listnum] - 1), $this->isunicode);
				break;
			}
			default: {
				$textitem = $this->listcount[$this->listnum];
		...
		if (!WMS_TCPDF_STATIC::empty_string($textitem)) {
			$prev_y = $this->y;
			$h = $this->getCellHeight($this->FontSize);
	...
	public function addTOC($page='', $numbersfont='', $filler='.', $toc_name='TOC', $style='', $color=array(0,0,0)) {
		...
		$page_fill_end = false;
		$current_column = $this->current_column;
		if (WMS_TCPDF_STATIC::empty_string($numbersfont)) {
			$numbersfont = $this->default_monospaced_font;
		}
		if (WMS_TCPDF_STATIC::empty_string($filler)) {
			$filler = ' ';
		}
		if (WMS_TCPDF_STATIC::empty_string($page)) {
			$gap = ' ';
		} else {
			$gap = '';
			if ($page < 1) {
		...
		if (!WMS_TCPDF_STATIC::empty_string($page)) {
			for ($p = $page_first; $p <= $page_last; ++$p) {
				...
				for ($n = 1; $n <= $maxpage; ++$n) {
					...
					$na = WMS_TCPDF_STATIC::formatTOCPageNumber(($this->starting_page_number + $np - 1));
					$nu = WMS_TCPDF_FONTS::UTF8ToUTF16BE($na, false, $this->isunicode, $this->CurrentFont);
					foreach ($pnalias['u'] as $u) {
						$sfill = str_repeat($filler, max(0, (strlen($u) - strlen($nu.' '))));
						if ($this->rtl) {
							$nr = $nu . WMS_TCPDF_FONTS::UTF8ToUTF16BE(' ' . $sfill, false, $this->isunicode, $this->CurrentFont);
						} else {
							$nr = WMS_TCPDF_FONTS::UTF8ToUTF16BE($sfill . ' ', false, $this->isunicode, $this->CurrentFont) . $nu;
						}
	...
	public function startTransaction() {
		...
		$this->start_transaction_page = $this->page;
		$this->start_transaction_y = $this->y;
		$this->objcopy = WMS_TCPDF_STATIC::objclone($this);
	}
	...
	protected function hyphenateWord($word, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		...
		$word_string = WMS_TCPDF_FONTS::UTF8ArrSubString($word, '', '', $this->isunicode);
		...
		if (isset($dictionary[$word_string])) {
			return WMS_TCPDF_FONTS::UTF8StringToArray($dictionary[$word_string], $this->isunicode, $this->CurrentFont);
		}
		...
		for ($pos = 0; $pos < $maxpos; ++$pos) {
			...
			for ($i = 1; $i <= $imax; ++$i) {
				$subword = strtolower(WMS_TCPDF_FONTS::UTF8ArrSubString($tmpword, $pos, ($pos + $i), $this->isunicode));
				if (isset($patterns[$subword])) {
					$pattern = WMS_TCPDF_FONTS::UTF8StringToArray($patterns[$subword], $this->isunicode, $this->CurrentFont);
	...
	public function hyphenateText($text, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		...
		if (!is_array($patterns)) {
			$patterns = WMS_TCPDF_STATIC::getHyphenPatternsFromTEX($patterns);
		}
		$unichars = WMS_TCPDF_FONTS::UTF8StringToArray($text, $this->isunicode, $this->CurrentFont);
		foreach ($unichars as $char) {
			if ((!$intag) AND (!$skip) AND WMS_TCPDF_FONT_DATA::$uni_type[$char] == 'L') {
				$word[] = $char;
			} else {
				if (!WMS_TCPDF_STATIC::empty_string($word)) {
		...
		return WMS_TCPDF_FONTS::UTF8ArrSubString($txtarr, '', '', $this->isunicode);
	}
	...
	public function printTemplate($id, $x='', $y='', $w=0, $h=0, $align='', $palign='', $fitonpage=false) {
		...
		if (!empty($this->xobjects[$id]['annotations'])) {
			foreach ($this->xobjects[$id]['annotations'] as $annot) {
				$coordlt = WMS_TCPDF_STATIC::getTransformationMatrixProduct($tm, array(1, 0, 0, 1, ($annot['x'] * $this->k), (-$annot['y'] * $this->k)));
				$ax = ($coordlt[4] / $this->k);
				$ay = ($this->h - $h - ($coordlt[5] / $this->k));
				$coordrb = WMS_TCPDF_STATIC::getTransformationMatrixProduct($tm, array(1, 0, 0, 1, (($annot['x'] + $annot['w']) * $this->k), ((-$annot['y'] - $annot['h']) * $this->k)));
	...
	public function ImageSVG($file, $x='', $y='', $w=0, $h=0, $link='', $align='', $palign='', $border=0, $fitonpage=false) {
		...
		if ($file[0] === '@') { // image from string
			...
			$svgdata = WMS_TCPDF_STATIC::fileGetContents($file);
		}
		if ($svgdata === FALSE) {
	...
	protected function setSVGStyles($svgstyle, $prevsvgstyle, $x=0, $y=0, $w=1, $h=1, $clip_function='', $clip_params=array()) {
		...
		$fill_color = WMS_TCPDF_COLORS::convertHTMLColorToDec($svgstyle['color'], $this->spot_colors);
		$this->SetFillColorArray($fill_color);
		$text_color = WMS_TCPDF_COLORS::convertHTMLColorToDec($svgstyle['text-color'], $this->spot_colors);
		...
		if (preg_match('/url\([\s]*\#([^\)]*)\)/si', $svgstyle['fill'], $regs)) {
			...
			$fill_color = WMS_TCPDF_COLORS::convertHTMLColorToDec($svgstyle['fill'], $this->spot_colors);
		...
		if ($svgstyle['stroke'] != 'none') {
			...
			$stroke_style = array(
				'color' => WMS_TCPDF_COLORS::convertHTMLColorToDec($svgstyle['stroke'], $this->spot_colors),
					...
					foreach ($params as $ck => $cp) {
						...
						if ((($ck + 1) % 7) == 0) {
							...
							if ((abs($x0 - $x) < $minlen) AND (abs($y0 - $y) < $minlen)) {
								...
								$angs = WMS_TCPDF_STATIC::getVectorsAngle(1, 0, (($xa - $cax) / $rx), (($cay - $ya) / $ry));
								$dang = WMS_TCPDF_STATIC::getVectorsAngle((($xa - $cax) / $rx), (($ya - $cay) / $ry), ((-$xa - $cax) / $rx), ((-$ya - $cay) / $ry));
								if (($fs == 0) AND ($dang > 0)) {
									$dang -= (2 * M_PI);
	...
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm=array()) {
		...
		if (isset($attribs['style']) AND !WMS_TCPDF_STATIC::empty_string($attribs['style']) AND ($attribs['style'][0] != ';')) {
			$attribs['style'] = ';'.$attribs['style'];
		}
		foreach ($prev_svgstyle as $key => $val) {
			if (in_array($key, WMS_TCPDF_IMAGES::$svginheritprop)) {
				$svgstyle[$key] = $val;
			}
			if (isset($attribs[$key]) AND !WMS_TCPDF_STATIC::empty_string($attribs[$key])) {
				if ($attribs[$key] == 'inherit') {
					$svgstyle[$key] = $val;
				} else {
					$svgstyle[$key] = $attribs[$key];
				}
			} elseif (isset($attribs['style']) AND !WMS_TCPDF_STATIC::empty_string($attribs['style'])) {
				$attrval = array();
		...
		if (isset($attribs['transform']) AND !empty($attribs['transform'])) {
			$tm = WMS_TCPDF_STATIC::getTransformationMatrixProduct($tm, WMS_TCPDF_STATIC::getSVGTransformMatrix($attribs['transform']));
		}
		...
		switch($name) {
			...
			case 'svg': {
				...
				$tm = WMS_TCPDF_STATIC::getTransformationMatrixProduct($tm, array(1, 0, 0, 1, $svgX, $svgY));
				...
				if (isset($attribs['viewBox'])) {
					...
					if (sizeof($tmp) == 4) {
						...
						$tm = WMS_TCPDF_STATIC::getTransformationMatrixProduct($tm, $newtm);
						$this->SVGTransform($tm);
					}
				}
			...
			case 'g': {
				...
				$tm = WMS_TCPDF_STATIC::getTransformationMatrixProduct($tm, array($w, 0, 0, $h, $x, $y));
				$this->SVGTransform($tm);
				$this->setSVGStyles($svgstyle, $prev_svgstyle);
				break;
			}
			case 'linearGradient': {
				...
				if (isset($attribs['gradientTransform'])) {
					$this->svggradients[$this->svggradientid]['gradientTransform'] = WMS_TCPDF_STATIC::getSVGTransformMatrix($attribs['gradientTransform']);
				}
			...
			case 'stop': {
				...
				$stop_color = isset($svgstyle['stop-color'])?WMS_TCPDF_COLORS::convertHTMLColorToDec($svgstyle['stop-color'], $this->spot_colors):'black';
			...
			case 'image': {
				...
				if (!$clipping) {
					...
					if (preg_match('/^data:image\/[^;]+;base64,/', $img, $m) > 0) {
						...
						if (!WMS_TCPDF_STATIC::empty_string($this->svgdir) AND (($img[0] == '.') OR (basename($img) == $img))) {
							$img = $this->svgdir.'/'.$img;
						}
						...
						if (empty($testscrtype['query'])) {
							$img = str_replace(WMS_K_PATH_URL, WMS_K_PATH_MAIN, $img);
						} elseif (preg_match('|^https?://|', $img) !== 1) {
							$img = str_replace(WMS_K_PATH_MAIN, WMS_K_PATH_URL, $img);
						}
					}
					$imgtype = WMS_TCPDF_IMAGES::getImageFileType($img);
					if (($imgtype == 'eps') OR ($imgtype == 'ai')) {
